...
try:
    ...
    print(json.dumps({'driver_error':'runtime did not exit after CTRL_BREAK','code':child.returncode,
                      'stdout':out,'stderr':err}))
    raise SystemExit(4)
...
'''
        ...
        result = subprocess.run([sys.executable, "-c", driver, self.runtime,
                                 str(self.config), str(cwd), str(self.ready)],
    ...
    def test_unmanaged_ctrl_break_reaches_vendor(self):
        ...
        outcome = self.drive_break(unmanaged)
    ...
    def test_managed_ctrl_break_is_forwarded_by_lifecycle(self):
        self.write_signal_config(self.topic)
        outcome = self.drive_break(self.topic)
        self.assertEqual(outcome["code"], 37, outcome)
        self.assertTrue(self.ready.with_suffix(".received").is_file())
