...
def _retirement_contents(workspace):
    ...
    if _snapshot(workspace) or git(workspace, 'submodule', 'status', '--recursive'):
        ...
        for name in [*dirs, *files]:
            ...
            if path.is_dir():
                if logical not in directories:
                    raise LifecycleError('retire refuses unowned directory content: ' + logical)
            elif logical not in tracked or not path.is_file():
