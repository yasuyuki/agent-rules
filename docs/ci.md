...
| Remote-only adoption with historical base, pinned-tip recovery and checkout conflict rejection | New `test_remote_*` cases in `test_branch_management` | Adoption/content preservation and both interrupted creation paths |
...
wheel-specific E2Es, including remote-only adoption and pinned-tip recovery. Separate runners isolate build output, source
