...
class ManagedRetirementStartTests(unittest.TestCase):
    def setUp(self):
        fixture_spec = importlib.util.spec_from_file_location('retirement_fixture', ROOT / 'tests/test_branch_management.py')
        fixture_module = importlib.util.module_from_spec(fixture_spec)
        fixture_spec.loader.exec_module(fixture_module)
        self.fixture = fixture_module.BranchManagementTests('runTest')
        self.addCleanup(self.fixture.doCleanups)
        self.fixture.setUp()
        self.fixture.begin('integration', 'adopt', branch='main', into='main')
        self.original_cwd = Path.cwd()
        self.addCleanup(os.chdir, self.original_cwd)
        self.workspaces = {'test': {'site': 'local', 'kind': 'direct', 'path': str(self.fixture.root)}}
        # Declare the primary repo; the actual launch may use a linked checkout.
        self.workspaces['repo'] = {'site': 'local', 'kind': 'direct', 'path': str(self.fixture.repo)}

    def request_args(self):
        return [sys.executable, str(ROOT / 'bin/place.py'), 'branch', 'retire',
                '--repo', str(self.fixture.repo), '--task', 'source', '--request',
                '--users-released', '--result-ref', 'https://example.invalid/saved-result', '--json']

    def test_owned_child_requests_retirement_and_exit_removes_checkout(self):
        script = 'import subprocess,json\np=subprocess.run(%r,capture_output=True,text=True)\nassert json.loads(p.stdout)["pending"], p.stdout+p.stderr\n' % self.request_args()
        tree = self.fixture.integrate('source', name='complete.py', contents=script)
        before = self.fixture.git('rev-parse', 'source').stdout
        with contextlib.redirect_stderr(io.StringIO()):
            code = place.run_registered([sys.executable, str(tree / 'complete.py')],
                                        {'cwd': str(tree)}, subprocess.run,
                                        self.workspaces, 'local', tree)
        self.assertEqual(code, 0)
        self.assertFalse(tree.exists())
        self.assertEqual(self.fixture.git('rev-parse', 'source').stdout, before)

    def test_next_start_replays_pending_before_invoking_vendor(self):
        tree = self.fixture.integrate('source')
        blocker = tree / 'preserved-user-file'
        blocker.write_text('keep until explicitly handled')
        self.fixture.command(*(arg for arg in self.request_args() if arg != '--request'), ok=False)
        ...
        with contextlib.redirect_stderr(io.StringIO()):
            code = place.run_registered(['vendor'], {'cwd': str(self.fixture.repo)}, vendor,
                                        self.workspaces, 'local', self.fixture.repo)
        self.assertEqual(code, 0)

    def test_failed_spawn_does_not_leave_an_unknown_child_lease(self):
        tree = self.fixture.integrate('source')
        with self.assertRaises(FileNotFoundError):
            place.run_registered([str(self.fixture.root / 'missing-vendor')], {'cwd': str(tree)},
                                 subprocess.run, self.workspaces, 'local', tree)
        state = json.loads((self.fixture.repo / '.git/agent-branches/state.json').read_text())
        self.assertFalse(state['tasks']['source'].get('leases'))

    @unittest.skipIf(os.name == 'nt', 'POSIX managed terminal and process groups')
    def test_registered_supervisor_forwards_interrupt_with_terminal(self):
        import pty
        import select
        import signal
        script = ('import os,signal,sys\n'
                  'signal.pthread_sigmask(signal.SIG_BLOCK,{signal.SIGINT})\n'
                  'print("TTY="+str(all(os.isatty(n) for n in (0,1,2))),flush=True)\n'
                  'signal.sigwait({signal.SIGINT})\nsys.exit(42)\n')
        tree = self.fixture.integrate('source', name='vendor.py', contents=script)
        launcher = ('import importlib.util,subprocess,sys\n'
                    's=importlib.util.spec_from_file_location("place",%r)\n'
                    'm=importlib.util.module_from_spec(s);s.loader.exec_module(m)\n'
                    'sys.exit(m.run_registered(%r,{"cwd":%r},m.exec_native,%r,"local",%r))\n') % (
                        str(ROOT / 'bin/place.py'), [sys.executable, str(tree / 'vendor.py')],
                        str(tree), self.workspaces, str(tree))
        master, slave = pty.openpty()
        child = None
        try:
            child = subprocess.Popen([sys.executable, '-c', launcher], stdin=slave, stdout=slave, stderr=slave)
            self.assertTrue(select.select([master], [], [], 20)[0])
            self.assertEqual(os.read(master, 4096).strip(), b'TTY=True')
            child.send_signal(signal.SIGINT)
            self.assertEqual(child.wait(timeout=20), 42)
            state = json.loads((self.fixture.repo / '.git/agent-branches/state.json').read_text())
            self.assertFalse(state['tasks']['source'].get('leases'))
        finally:
            if child is not None and child.poll() is None:
