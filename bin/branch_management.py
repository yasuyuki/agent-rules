...
def begin(args):
    ...
    if from_remote and args.mode != 'adopt':
        raise BranchError('--from-remote is only valid with --mode adopt')
    ...
    with locked(repo) as (directory, state):
        ...
        if args.mode == 'continue':
            ...
            if from_remote and any(p.is_symlink() for p in
                                   (Path(args.worktree), *Path(args.worktree).parents)):
            ...
            if args.depends_on:
                ...
                base = None  # Historical base is supplied explicitly below.
            else:
            ...
            if args.mode == 'new':
                ...
                if from_remote:
                    if Path(path).exists() or git(repo, 'rev-parse', '--verify', 'refs/heads/' + args.branch, optional=True):
                        raise BranchError('remote adoption requires an absent local branch and worktree path')
                    if git(repo, 'remote', 'get-url', '--all', state['remote']).splitlines() != [state['remote_url']]:
                        raise BranchError('registered remote must have exactly one fetch URL')
    ...
    if creation:
        ...
        if existing and existing != creation['tip']:
            raise BranchError('interrupted creation branch tip changed; preserve and inspect')
        ...
        for name in ('install', 'begin', 'check', 'retire', 'prepare-merge', 'allow-cherry-pick', 'allow-tag-push'):
            ...
            if name == 'install':
                ...
                p.add_argument('--from-remote', action='store_true',
                               help='adopt a fetched remote branch into a new local branch/worktree')
