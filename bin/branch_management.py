    ...
    with locked(repo) as (directory, state):
        ...
        if args.mode == 'continue':
            ...
            if task.get('creating'):
                if args.sync:
                    raise BranchError('finish interrupted creation before syncing')
                creation = task
            else:
            ...
            raw_path = Path(args.worktree).absolute()
            if from_remote and any(p.is_symlink() for p in (raw_path, *raw_path.parents)):
            ...
            if args.mode == 'new':
                ...
                if from_remote:
                    if Path(path).exists() or git(repo, 'rev-parse', '--verify', 'refs/heads/' + args.branch, optional=True):
                        raise BranchError('remote adoption requires absent local branch and worktree')
                    remote_ref = 'refs/remotes/' + state['remote'] + '/' + args.branch
                    tip = oid(repo, remote_ref)
                ...
                remote_tip = tip if from_remote else git(repo, 'rev-parse', '--verify', 'refs/remotes/' + state['remote'] + '/' + args.branch, optional=True)
