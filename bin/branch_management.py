...
def checkout(repo, state):
    ...
    if task.get('creating'):
        raise BranchError('finish interrupted creation with --mode continue before updating work')
...
def begin(args):
    ...
    with locked(repo) as (directory, state):
        ...
        if args.mode == 'continue':
            ...
            if task.get('creating'):
                if args.sync:
                    raise BranchError('finish interrupted creation before importing updates')
                creation = task
            else:
            ...
            if from_remote and (Path(args.worktree).exists() or Path(args.worktree).is_symlink()):
            ...
            if args.mode == 'new':
                ...
                if from_remote:
                    ...
                    ref = 'refs/remotes/' + state['remote'] + '/' + args.branch
                    tip = oid(repo, ref)
                    if git(repo, 'cat-file', '-t', ref) != 'commit':
                        raise BranchError('remote tracking ref is not a commit')
                    advertised = git(repo, 'ls-remote', state['remote'], 'refs/heads/' + args.branch).split()
                    if not advertised or advertised[0] != tip:
                        raise BranchError('fetch the remote branch before adoption; tracking ref differs from advertised commit')
                else:
    ...
    if creation:
        ...
        if Path(creation['worktree']).is_symlink():
            raise BranchError('interrupted creation has a symlink collision')
